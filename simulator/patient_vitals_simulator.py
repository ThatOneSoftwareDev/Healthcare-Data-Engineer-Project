...
PATIENT_COUNT = int(os.getenv("PATIENT_COUNT", 20)) # just making sure we get an int
STREAM_INTERVAL = int(os.getenv("STREAM_INTERVAL", 2)) # just making sure we get an int
ERROR_RATE = float(os.getenv("ERROR_RATE", 0.1))  # fraction of error records , making sure we get a float

# Pub/Sub client
publisher = pubsub_v1.PublisherClient()
topic_path = publisher.topic_path(PROJECT_ID, TOPIC_ID) # our streaming data

# Generate list of patient IDs
patient_ids = [f"P{i:03d}" for i in range(1, PATIENT_COUNT + 1)]

# everythime generate vitals is called it creates a single record
def generate_vitals():
